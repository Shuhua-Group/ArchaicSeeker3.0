...
ASEEK_EXEC="/path/to/your/ArchaicSeeker3.1-mamba"
...
DATA_IN_PATH="/path/to/your/preprocessing_output"

# 3. Root directory for the results of this ArchaicSeeker3 analysis.
#    A timestamp will be appended to this path.
ANALYSIS_OUT_ROOT="/path/to/your/aseeker3_results_$(date +%Y%m%d)"
  ...
  (
    ...
    for K in {1..22}; do
      if (( (K-1) % NGPUS == GPU )); then
        ...
        target_vcf="${TARGET_VCF_DIR}/HGDPTGP_target.chr${K}.vcf.gz"
        ref_vcf="${REF_VCF_DIR}/Archaic.AFR.ref.chr${K}.vcf.gz"
