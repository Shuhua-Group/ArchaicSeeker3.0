...
summary_out = os.path.join(output_dir, "inference/ArchaicSeeker3.1/AS3_Mamba_Smoother_4096_2048_M5216_2src_accuracy.txt")
        ...
        for nref in nref_list:
            for ntgt in ntgt_list:
                for seed in seed_list:
                    ...
                    try:
                        prefix = os.path.join(output_dir, f"inference/ArchaicSeeker3.1/{demog}/nref_{nref}/ntgt_{ntgt}/{seed}")
                        seg_file = os.path.join(prefix, "AS3_Mamba_Smoother_4096_2048_M5216.bed")
                        ...
                        bed_out_1 = os.path.join(prefix, f"AS3_Mamba_Smoother_4096_2048_M5216.out.cutoff.{cutoff}.src1.bed")
                        bed_out_2 = os.path.join(prefix, f"AS3_Mamba_Smoother_4096_2048_M5216.out.cutoff.{cutoff}.src2.bed")
                        bed_out = os.path.join(prefix, f"AS3_Mamba_Smoother_4096_2048_M5216.out.cutoff.{cutoff}.bed")
                        acc_out = os.path.join(prefix, f"AS3_Mamba_Smoother_4096_2048_M5216.out.cutoff.{cutoff}.accuracy")
                        ...
                        precision_11, recall_11 = cal_accuracy(true_file_1, bed_out_1)
                        precision_12, recall_12 = cal_accuracy(true_file_1, bed_out_2)
                        precision_21, recall_21 = cal_accuracy(true_file_2, bed_out_1)
                        precision_22, recall_22 = cal_accuracy(true_file_2, bed_out_2)
                        precision, recall = cal_accuracy(merged_true_file, bed_out)

                        # 写入主输出文件
                        fout.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc11\t{cutoff}\t{precision_11:.2f}\t{recall_11:.2f}\n")
                        fout.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc12\t{cutoff}\t{precision_12:.2f}\t{recall_12:.2f}\n")
                        fout.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc21\t{cutoff}\t{precision_21:.2f}\t{recall_21:.2f}\n")
                        fout.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc22\t{cutoff}\t{precision_22:.2f}\t{recall_22:.2f}\n")
                        fout.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc\t{cutoff}\t{precision:.2f}\t{recall:.2f}\n")

                        # 写入精简 acc 文件
                        with open(acc_out, 'w') as acc_f:
                            acc_f.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc11\t{cutoff}\t{precision_11:.2f}\t{recall_11:.2f}\n")
                            acc_f.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc12\t{cutoff}\t{precision_12:.2f}\t{recall_12:.2f}\n")
                            acc_f.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc21\t{cutoff}\t{precision_21:.2f}\t{recall_21:.2f}\n")
                            acc_f.write(f"{demog}\tnref_{nref}_ntgt_{ntgt}\tsrc22\t{cutoff}\t{precision_22:.2f}\t{recall_22:.2f}\n")
