...
from utils.utils import process_skovhmm_output_2src_new, cal_accuracy
...
inferred_tracts_file1 = os.path.join(skovhmm_dir, "inferred_tracts1.bed")
inferred_tracts_file2 = os.path.join(skovhmm_dir, "inferred_tracts2.bed")
...
process_skovhmm_output_2src_new(merged_hap_file, inferred_tracts_file,inferred_tracts_file1, inferred_tracts_file2, cutoff=threshold)

# 计算准确率
true_tracts_file = os.path.join(output_dir, f"simulated_data/{demog}/nref_{nref}/ntgt_{ntgt}/{seed}/sim2src.introgressed.tracts.bed")
true_tracts_file1 = os.path.join(output_dir, f"simulated_data/{demog}/nref_{nref}/ntgt_{ntgt}/{seed}/sim2src.src1.introgressed.tracts.bed")
true_tracts_file2 = os.path.join(output_dir, f"simulated_data/{demog}/nref_{nref}/ntgt_{ntgt}/{seed}/sim2src.src2.introgressed.tracts.bed")
precision1, recall1 = cal_accuracy(true_tracts_file1, inferred_tracts_file1)
precision2, recall2 = cal_accuracy(true_tracts_file2, inferred_tracts_file2)
precision, recall = cal_accuracy(true_tracts_file, inferred_tracts_file)

# 写入accuracy
print(f"📝 Writing accuracy to {accuracy_file}...")
with open(accuracy_file, 'w') as f:
    f.write(f'{demog}\tnref_{nref}_ntgt_{ntgt}\t{threshold}\tsrc1\t{precision1:.4f}\t{recall1:.4f}\n')
    f.write(f'{demog}\tnref_{nref}_ntgt_{ntgt}\t{threshold}\tsrc2\t{precision2:.4f}\t{recall2:.4f}\n')
