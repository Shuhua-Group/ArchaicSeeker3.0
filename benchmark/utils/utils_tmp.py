...
def process_skovhmm_output_2src_new(in_file, out_file1, out_file2, cutoff = 0.5):
    """
    Description:
        Helper function for converting output from SkovHMM to BED format given a cutoff.
    """
    df = pd.read_csv(in_file, sep="\t",header=0)
    df = df[df["state"] == 'Archaic']
    df["mean_prob"] = pd.to_numeric(df["mean_prob"], errors="coerce")
    df = df[df["mean_prob"] > cutoff]
    df.columns.values[-1] = 'src2'
    df.columns.values[-2] = 'src1'
    if df.columns[-3] != 'admixpopvariants':
        df.columns.values[-3] = 'intermediate'
    df['state'] = df[['src1', 'src2']].apply(lambda row: 'src1' if row['src1'] >= row['src2'] else 'src2', axis=1)
    def assign_state(row):
        src1_val = row['src1']
        src2_val = row['src2']
        interm_val = row['intermediate']
        max_val = max(src1_val, src2_val, interm_val)
        if max_val == interm_val:
            return 'src1'
        elif max_val == src1_val:
            return 'src1'
        else:
            return 'src2'
    df['state'] = df.apply(assign_state, axis=1)

    df1 = df[df['state'] == 'src1']
    df2 = df[df['state'] == 'src2']
    cols = ['chrom', 'start', 'end']
    df1.to_csv(out_file1, columns=cols, sep="\t", header=False, index=False)
    df2.to_csv(out_file2, columns=cols, sep="\t", header=False, index=False)
...
def process_as2_1src_output(in_file, out_file):
    """
    Description:
        Helper function for converting output from ArchaicSeeker 2.0 to BED format files.
    """
    df = pd.read_csv(in_file, sep="\t")
    df = df[df['BestMatchedPop'] == 'src']
    cols = ['Contig', 'Start(bp)', 'End(bp)']
    df.to_csv(out_file, columns=cols, sep="\t", header=False, index=False)


def process_as2_1src_10tgt_output(in_file):
    """
    Description:
        Helper function for converting output from ArchaicSeeker 2.0 to BED format files.
    """
    df = pd.read_csv(in_file, sep="\t")
    df = df[df['BestMatchedPop'] == 'src']
    df['ID_prefix'] = df['ID'].str.extract(r'(tsk_\d+)')
    for prefix, group_df in df.groupby('ID_prefix'):
        output_path = f'{prefix}.bed'
        group_df[['Contig', 'Start(bp)', 'End(bp)']].to_csv(output_path, sep='\t', header=False, index=False)
