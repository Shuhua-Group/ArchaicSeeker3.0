...
def merge_Den_bed(Den1_bed_path, Den2_bed_path, Den_bed_path, maxLen = 250):
    Den1_bed_raw = pd.read_csv(Den1_bed_path, sep="\t", header=None)
    Den1_bed_raw = Den1_bed_raw[Den1_bed_raw[2] <= maxLen * 1000000]
    Den2_bed_raw = pd.read_csv(Den2_bed_path, sep="\t", header=None)
    Den2_bed_raw = Den2_bed_raw[Den2_bed_raw[2] <= maxLen * 1000000]
    Den_bed_raw = pd.concat([Den1_bed_raw, Den2_bed_raw])
    Den_bed_raw.to_csv(Den_bed_path, sep="\t", header=False, index=False)

def process_Nean_bed(Nean_raw_bed_path, Nean_bed_path, maxLen = 250):
    Nean_raw_bed_raw = pd.read_csv(Nean_raw_bed_path, sep="\t", header=None)
    Nean_raw_bed_raw = Nean_raw_bed_raw[Nean_raw_bed_raw[2] <= maxLen * 1000000]
    Nean_raw_bed_raw.to_csv(Nean_bed_path, sep="\t", header=False, index=False)

def process_Archaic_bed(Den_bed_path, Nean_bed_path, archaic_bed_path, archaic_bed_info_path, maxLen = 250):
    Den_bed_raw = pd.read_csv(Den_bed_path, sep="\t", header=None)
    Den_bed_raw = Den_bed_raw[Den_bed_raw[2] <= maxLen * 1000000]
    Den_bed_raw[4] = "Den"
    Nean_bed_raw = pd.read_csv(Nean_bed_path, sep="\t", header=None)
    Nean_bed_raw = Nean_bed_raw[Nean_bed_raw[2] <= maxLen * 1000000]
    Nean_bed_raw[4] = "Nean"
...
def parse_args():
    ...
    parser.add_argument("--maxLen", type=int, required=True, help="maxLen")
...
def main():
    ...
    # 输入文件夹
    indir = Path(f"/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/2_Data/3_LongSeg/{seed}")
    # 输出文件夹
    outdir = Path(f"/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/2_Data/3_LongSeg/{seed}")
    ...
    Nean_raw_bed_path = indir / "Nean.sim.bed"

    merge_Den_bed(Den1_bed_path, Den2_bed_path, Den_bed_path, maxLen)
    process_Archaic_bed(Den_bed_path, Nean_bed_path, Archaic_bed_path, Archaic_bed_info_path, maxLen)

    # # 输出文件
    Target_vcf_path = outdir / f"target.Max{maxLen}Mb.vcf.gz"
    Ref_vcf_path = outdir / f"ref.Max{maxLen}Mb.vcf.gz"
    Ref_map_path = outdir / f"ref.Max{maxLen}Mb.map"
    Den_bed_path = outdir / f"Den.sim.Max{maxLen}Mb.bed"
    Nean_bed_path = outdir / f"Nean.sim.Max{maxLen}Mb.bed"
    Archaic_bed_path = outdir / f"Archaic.sim.Max{maxLen}Mb.bed"
    Archaic_bed_info_path = outdir / f"Archaic.sim.Max{maxLen}Mb.info"

    merge_Den_bed(Den1_bed_path, Den2_bed_path, Den_bed_path, maxLen)
    process_Nean_bed(Nean_raw_bed_path, Nean_bed_path, maxLen)
    process_Archaic_bed(Den_bed_path, Nean_bed_path, Archaic_bed_path, Archaic_bed_info_path, maxLen)

    # # 数据处理
    region = f"1:1-{maxLen}000000"
    subprocess.run(f"bcftools view -r {region} -Oz -o {Target_vcf_path} {Target_vcf}", shell=True)
    subprocess.run(f"bcftools index -t {Target_vcf_path}", shell=True)
    subprocess.run(f"bcftools view -r {region} -Oz -o {Ref_vcf_path} {Ref_vcf}", shell=True)
