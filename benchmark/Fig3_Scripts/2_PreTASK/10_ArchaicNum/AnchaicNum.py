...
def main():
    ...
    # 输出文件夹
    outdir = Path(f"/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/2_Data/10_ArchaicNum/{seed}")
    ...
    tgt_list = "/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/1_Scripts/2_PreTASK/8_RefNum/tgt.list"
    Target_vcf_path = outdir / "target.vcf.gz"

    subprocess.run(f"cd {outdir}", shell=True)
    subprocess.run(f"bcftools view -S {tgt_list} -Oz -o {Target_vcf_path} {Target_vcf}", shell=True)
    subprocess.run(f"bcftools index -t {Target_vcf_path}", shell=True)

    Ref_vcf = indir / "ref.vcf.gz"
    for archaic_num in ['n1d1','n2d1', 'n3d1', 'n3d2', 'n3d3', 'n3d5', 'n3d10', 'n5d1', 'n5d2', 'n5d5', 'n10d1', 'n10d2', 'n10d10']:
        ref_list = f"/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/1_Scripts/2_PreTASK/10_ArchaicNum/ref_{archaic_num}.list"
        ref_map = f"/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/1_Scripts/2_PreTASK/10_ArchaicNum/ref_{archaic_num}.map"
        Ref_vcf_path = outdir / f"ref_{archaic_num}.vcf.gz"
        Ref_map_path = outdir / f"ref_{archaic_num}.map"
