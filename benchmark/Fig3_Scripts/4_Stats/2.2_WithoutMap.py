...
def Archaic2DenNean(Archaic_infered_bed, Archaic_infered_tmp_bed,Den_infered_bed, Nean_infered_bed):
    ...
    df[df[6] == 1][[0,1,2,5]].to_csv(Den_infered_bed, sep="\t", header=None, index=False)
    df[df[6] == 2][[0,1,2,5]].to_csv(Nean_infered_bed, sep="\t", header=None, index=False)
    df[[0,1,2,5]].to_csv(Archaic_infered_tmp_bed,sep='\t',header=None,index=False)
...
def main():
    ...
    basedir = Path("/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/2_Data/2_WithMap/")
