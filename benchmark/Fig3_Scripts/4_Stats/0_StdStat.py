...
def main():
    ...
    best_matches_infersim = get_best_matches(Archaic_infered_bed, Archaic_sim_info_bed, out_dir, prefix)
    FP_seg_all, FP_seg_1, FP_seg_2, FP_seg_3, T0_P1, T0_P2, T0_P3, T1_P1, T1_P2, T1_P3, T2_P1, T2_P2, T2_P3, acc = analyze_infer_sim_13(best_matches_infersim, min_overlap_sim_ratio=0)


    with open(out_accuracy, "w") as f:
        f.write(f"{prefix}\t{seed}\t{Archaic_prec}\t{Archaic_rec}\t{Archaic_f1}\t{Archaic_ratio}\t{Den_prec}\t{Den_rec}\t{Den_f1}\t{Den_ratio}\t{Nean_prec}\t{Nean_rec}\t{Nean_f1}\t{Nean_ratio}\t{FP_seg_all}\t{FP_seg_1}\t{FP_seg_2}\t{FP_seg_3}\t{acc}\t{T0_P1}\t{T0_P2}\t{T0_P3}\t{T1_P1}\t{T1_P2}\t{T1_P3}\t{T2_P1}\t{T2_P2}\t{T2_P3}\n")
