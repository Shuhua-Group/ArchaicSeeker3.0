...
def parse_args():
    ...
    parser.add_argument("--cutoff", type=float, required=True, help="cutoff")
    return parser.parse_args()

def Archaic2DenNean(Archaic_infered_bed,Archaic_infered_bed_score,Den_infered_bed, Nean_infered_bed, Score_cutoff = 0.5):
    df = pd.read_csv(Archaic_infered_bed, sep="\t", header=None)
    df = df[df[6] >= Score_cutoff]
    df.to_csv(Archaic_infered_bed_score, sep="\t", header=None, index=False)
...
def main():
    ...
    Archaic_infered_bed = out_dir / f"AS3_Merge_0.bed"
    Archaic_infered_bed_score = out_dir / f"AS3_Merge_0.{prefix}.bed"
    Den_infered_bed = out_dir / f"Den.AS3_Merge_0.{prefix}.bed"
    Nean_infered_bed = out_dir / f"Nean.AS3_Merge_0.{prefix}.bed"

    out_accuracy = out_dir / f"{prefix}.accuracy"
    
    Archaic2DenNean(Archaic_infered_bed, Archaic_infered_bed_score, Den_infered_bed, Nean_infered_bed, Score_cutoff = cutoff)

    Archaic_prec, Archaic_rec, Archaic_f1, Archaic_ratio = cal_accuracy_hap(Archaic_sim_bed, Archaic_infered_bed_score)  
