...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tgt_num", type=int, required=True, help="tgt_num")
...
def main():
    args = parse_args()
    seed = args.seed
    tgt_num = args.tgt_num

    basedir = Path("/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/2_Data/9_TgtNum/")
    out_dir = basedir / str(seed) 
    res_dir = basedir / str(seed) / str(tgt_num)
    prefix = f"{tgt_num}"

    print(f"seed: {seed}")
    print(f"prefix: {prefix}")

    Archaic_sim_bed = out_dir / f"Archaic.sim.{tgt_num}.bed"
    Archaic_sim_info_bed = out_dir / f"Archaic.sim.{tgt_num}.info.bed"
    Den_sim_bed     = out_dir / f"Den.sim.{tgt_num}.bed"
    Nean_sim_bed    = out_dir / f"Nean.sim.{tgt_num}.bed"
