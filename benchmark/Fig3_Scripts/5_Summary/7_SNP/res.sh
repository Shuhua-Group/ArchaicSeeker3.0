...
base_dir="/home/linhuanyu/share1/25_AS_Real/ArchaicSeeker3/500.Simulation_10J19/2_Data/7_SNP"
...
for seed_dir in "${base_dir}"/*/; do
    ...
    for ru in mask*_resource_usage.tsv; do
        ...
        # ④ 横向拼成一行写入 maskX.res
        rm -f "${mask}.res"
        echo -e "${ru_line}\t${acc_line}\t${snp}" > "${mask}.res"
    done
done
